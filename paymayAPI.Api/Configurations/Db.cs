...
namespace paymayAPI.Api.Configurations
{
    public static class Db
    {
        internal static void RegisterEntityFramework(IServiceCollection services, IConfiguration configuration)
        {
            services.AddDbContext<paymayAPIDbContext>(opts =>
            {
                opts.UseSqlServer(configuration.GetConnectionString("Default"));
            });
        }

        internal static void ConfigureDatabaseMigrations(paymayAPIDbContext context)
        {
